...
- 机场：给每个机场命名，编辑它的筛选和重命名规则；可以添加或删除机场
- 三条 Clash 订阅：`Clash SS`、`Clash AnyTLS`、`Clash SS 2`，每条填机场订阅链接并选择所属机场
...
筛选和重命名规则按“机场”管理，同一机场下的订阅共用同一套规则。默认两个机场：

- 机场一（`Clash SS`、`Clash AnyTLS` 默认归属）：
...
在“机场”卡片里可以改名、改规则、加减机场，下方会显示已解码的规则预览，方便检查正则。每条订阅在“订阅设置”里用下拉框选择所属机场。`target=clash` 和 `url=你的机场订阅链接` 由程序自动生成，不需要手动填写。

### 一起更新或单独更新

顶部的“立即更新全部”会刷新所有已填写订阅链接的订阅。每条订阅的卡片上还有“更新此订阅”，只刷新这一条，其它订阅的状态保持不变。
